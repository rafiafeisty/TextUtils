    ...
    <nav className={`navbar navbar-expand-lg navbar-${props.mode} bg-${props.mode}`}>
      ...
      <div className="collapse navbar-collapse" id="navbarSupportedContent">
        ...
        <div className={`custom-control mx-2 custom-switch text-${props.mode==="light"?"dark":"light"}`}>
          <input type="checkbox" className="custom-control-input" id="darkModeSwitch" onClick={props.toggleMode}/>
          <label className="custom-control-label" htmlFor="darkModeSwitch">Enable Dark Mode</label>
        </div>
      </div>
    </nav>
  )
